...
- **Letzte Änderung:** IDrive e2 Region/Buckets/Access-Key angelegt, Spaceship-DNS fuer `cdn` und `uploads` gesetzt, Salad-Organisation erstellt und lokale Infrastruktur-Konfiguration aktualisiert.
...
- [x] IDrive e2 Region `us-west-2`, Buckets `swahh-media`/`swahh-private` und Access Key `swahh-control-server` erstellt.
